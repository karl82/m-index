...
& Typ & DDR3 \\ \cline{2-3}
& Frekvence & \SI{1.333}{\MHz} \\ \cline{2-3}
\hline
...
počet objektů pro porovnání & 68.040 \\ \hline
dimenze objektů & 32 \\ \hline
\end{tabular}
\end{table}

Původním záměrem bylo změřit více hodnot, např. počet pivotů 100 s~max. počtem objektů v~listech 10 a úrovní clusteru 5.
Při těchto parametrech byl ovšem počet clusterů extrémně velký -- $100 \times 99 \times 98 \times 97 \times 96 = 9.034.502.400$, a docházelo k~přetečení typu \type{Integer} v~kolekcích a k~nesmyslným výsledkům.\footnote{V~Java je typ \type{Integer} 4 bajtový celočíselný typ se znaménkem a může mít maximálně hodnotu $2^{31}-1$}

Při tomto množství je celkový počet kombinací parametrů 252 pro dynamický cluster a 36 pro vícestupňový cluster.
Jelikož počet měření je 20 pro každý parametr, bylo celkově vykonáno 5.760 měření.
...
Nejlepšího výsledku, tzn.~v~nejkratším čase, bylo dosaženo s~dynamickým \MIndex{em} s~následujícími parametry: počet pivotů 10, maximální úrovní clusteru 4, maximálním počtem objektů v~listech 10 a stupněm \BPTree{} 500.
Výsledný čas byl \SI{3.485}{\ms} pro parametry JVM1.
...
Nejdelší zpracování dotazů bylo u~vícestupňového \MIndex{u} pro JVM4 s~parametry: počet pivotů 50, maximální úrovní clusteru 4 a stupněm \BPTree{} 10.
Výsledný čas je \SI{5.201}{\ms} a rozdíl oproti nejkratší konfiguraci je více jak \SI{1.700}{\ms}.
...
Pro zobrazení zásadního vlivu nastavení JVM je i zobrazen vícestupňový \MIndex{} pro 50 pivotů a ú
\begin{figure}[t]
\centering
...
\hline
JVM1 & \SI{3.961}{\ms} \\ \hline
JVM2 & \SI{4.205}{\ms} \\ \hline
JVM3 & \SI{4.206}{\ms} \\ \hline
JVM4 & \SI{4.524}{\ms} \\ \hline
\end{tabular}
\caption{Průměrný čas měření}
\label{tab:jvmavgtime}
\end{table}

Obecně lze konstatovat, že konfigurace s~menším počtem pivotů (10), s~větší úrovní clusteru (4) a s~větším stupněm \BPTree{} (500) dosahovaly kratších časů.
...
Náklady při filtrování většího množství pivotů v~\emph{Range Query} je zřejmě signifikantní a neefektivní.
...
Parametry \MIndex{u} byly: vícestupňový cluster 3, počet pivotů 10 a stupeň \BPTree{} 50.

Rozhodl jsem se pro porovnání použít konfiguraci s~nejlepším časem dosaženým v~Java -- dynamický \MIndex{} s~maximálním počtem 10 objektů v~listu a JVM1, a také s~nejlepším výsledkem při použit stupně \BPTree{} 500.
...
\hline
\CC & 50 & \SI{2.432}{\ms} \\ \hline
\CC{} (\cmd{-O3}) & 50 & \SI{697}{\ms} \\ \hline
\CC{} (\cmd{-O3 -march=native}) & 50 & \SI{408}{\ms} \\ \hline
Java JVM1 & 50 &  \SI{3.979}{\ms} \\ \hline
Java JVM1 & 500 &  \SI{3.500}{\ms} \\ \hline
\end{tabular}
...
Volba \cmd{-O3} povoluje např. \emph{loop unrolling}.
Volbou \cmd{-march=native} kompilátor používá všechny dostupné instrukce na dané architektuře.
Při kombinaci obou dvou voleb byl výsledný čas 6-krát menší než s~výchozími volbami kompilátoru (\cmd{-O2 -g}).
...
\cmd{cppcheck} nalezl celkem 8 chyb při práci s~dealokováním paměti. Zde je výstup:
{\tiny
\begin{verbatim}
...
Při bližší analýze kódu, byla většina chyb způsobena tím, že se pole, alokované pomocí operátoru \method{new[]}, uvolňovaly pomocí operátoru \method{delete} místo \method{delete[]}\cite{Stroustrup:1997:CPL:523265}.